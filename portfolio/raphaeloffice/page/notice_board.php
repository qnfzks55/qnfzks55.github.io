<?php
if($_GET[notice_board]){
	include $_GET[notice_board].".php";
}else{
	// 페이지화
...
<table cellpadding="0" cellspacing="0" class="table00">
	...
	<?php
		if(!$_GET[category]){
			$que = "select * from board where main_id='$_GET[main_id]' and sub_id='$_GET[sub_id]' order by no desc limit $start,$page";
		}elseif($_GET[category]){
			$que = "select * from board where main_id='$_GET[main_id]' and sub_id='$_GET[sub_id]' and category='$_GET[category]' order by no desc limit $start,$page";
		}
	...
	<tr>
		...
		<td class="table00_td"><span class="board_view"><a href="sub.php<?php echo PAGE; ?>&amp;notice_board=notice_board_view&amp;no=<?php echo $data[no]; ?>"><?php echo $data[subject]; ?></a></span></td>
	...
	<tr>
		<td colspan="6" style="text-align:right; padding:7px 5px 7px 7px;">
			<input type="button" class="button00" value="글쓰기" onclick="location.href='sub.php<?php echo PAGE; ?>&amp;notice_board=notice_board_write'" />
		</td>
	</tr>
</table>
